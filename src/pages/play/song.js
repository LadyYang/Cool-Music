...
(function (root, factory) {
    ...
    function syncLyricsTime(lyArr) {
        rollingLyrics(lyArr, lyricUiDom);
        renderTime(audio.currentTime, startTimeDom);
    ...
    function render(data) {
        readyImage(imgDom, data.img);
        // readyImage(pImg, data.img);
        renderSongInfo(songNameDom, authorDom, data);
        renderTime(audio.duration, endTimeDom);
        renderLyrics(data.lyrics, lyricUiDom);
    }
    ...
    Song.prototype.extend({
        ...
        bindEvent() {
            ...
            // click show lyrics
            dialDom.onclick = function () {
                this.style.opacity = 0;
                this.style.display = 'none';
                lyricUiDom.parentNode.style.display = 'block';
                lyricUiDom.parentNode.style.opacity = 1;
            }

            // click show dial
            lyricUiDom.parentNode.onclick = function () {
                this.style.opacity = 0;
                this.style.display = 'none';
                dialDom.style.display = 'block';
                dialDom.style.opacity = 1;
            }


            this.inputDom.addEventListener('input', this.debounce(this.loadSongList, 300));

            // Click on a single song
            this.songHash = [];
            singer.addEventListener('click', this.clickSongList.bind(this));
            ...
            playBtn.onclick = () => {
                if (this.isPlay) {
                    this.isPlay = false;
                    dialDom.classList.add('paused');
                    // pImg.classList.add('paused');
                    playBtn.classList.remove('pause-btn');
                    this.pause();
                } else {
                    this.isPlay = true;
                    this.play();
                    dialDom.classList.remove('paused');
                    // pImg.classList.remove('paused');
