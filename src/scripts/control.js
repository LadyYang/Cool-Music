(function () {
    ...
    this.doJSON = function doJSON(data) {
        cb.call(song, data);
    }
    ...
    function bindEvent() {
        if (flag) {
            ...
            this.inputDom.oninput = debounce(this.loadSongList, 300);
            ...
            singer.onclick = (e) => {
                ...
                if (target.className === 'singer_name') {
                    ...
                    let hash = self.songList[self.index].FileHash;
            ...
            var leftWrapper = document.querySelector('.ct .left-wrapper');
            var baffle = document.querySelector('.right-wrapper .baffle');
            menu.flag = false;
            menu.onclick = () => {
                if (menu.flag) {
                    leftWrapper.style.left = '-100%';
